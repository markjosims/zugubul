...
def train(
    ...
    if audio_cutoff is not None:
        ...
        print(f"Length of dataset before filtering: train: {len(dataset['train'])}, validation: {len(dataset['validation'])}, test: {len(dataset['test'])}")
        dataset = dataset.filter(lambda r: r['audio']['array'].shape[0]<audio_cutoff)
        print(f"Length of dataset after filtering: train: {len(dataset['train'])}, validation: {len(dataset['validation'])}, test: {len(dataset['test'])}")
